...
public class DamageTargetStats : MonoBehaviour
{
    ...
    public bool IsBlocking => (Input.GetKeyDown(KeyCode.J) || !IsAttacking) && blockUses > 0;
            ...
            if (c.CompareTag("jugador1"))
            {
                ...
                if(c.GetComponent<Attack>().IsBlocking)
                {
                    c.GetComponent<KnockbackEffect>().CallKnockback(
                        (-hitDirection * facingDirection) / 2, new Vector2(-facingDirection * 0.1f, 0.06f), Input.GetAxisRaw("Horizontal")
                    );
                    blockUses--;
                } else
                {
